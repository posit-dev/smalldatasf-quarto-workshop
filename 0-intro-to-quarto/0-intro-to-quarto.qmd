...
## Setup, continued {.smaller}

::: task

**3. Exercises:** Go to [`pos.it/smalldatasf-quarto-exercises`](https://pos.it/smalldatasf-quarto-exercises), click on *Use this template* then *Create a new repository*.
...
````{.markdown code-line-numbers=|8-16}
```
title: "My first Quarto dashboard"
format: html
```
...
```{{python}}
#| include: false
#| fig.alt: "A scatterplot with cty on the x-axis and hwy on the y-axis."
from plotnine import ggplot, aes, geom_point
from plotnine.data import mpg

(
    ggplot(mtcars, aes(x = "cty", y = "hwy"))
    + geom_point()
)
```
...
::: task
In VS Code, open the `olympicdash-0.ipynb` file. Render the document to HTML.
